...
from wtforms import StringField, PasswordField, SelectField, FloatField, SubmitField, FieldList, DateField, FormField, DecimalField, TextAreaField, IntegerField, HiddenField
...
class FarmerSignUpForm(FlaskForm):
    ...
    farm_name = StringField('Farm Name', validators=[Optional(), Length(max=128)])
...
class AssignTaskForm(FlaskForm):
    ...
    labour_types = SelectField('Labour Type', choices=[], validators=[DataRequired()])
    employee = SelectField('Assign To', choices=[], validators=[DataRequired()])
...
class TrackTaskForm(FlaskForm):
    ...
    status = SelectField('Status', choices=[('completed', 'Completed'), ('in_progress', 'In Progress')], validators=[DataRequired()])
...
class ProductionForm(FlaskForm):
    ...
    rate = FloatField('Rate per kg', validators=[DataRequired(), NumberRange(min=0.01)])
...
class ProductionFilterForm(FlaskForm):
    ...
    year = SelectField('Select Year', choices=[], default=str(datetime.today().year))

    submit = SubmitField('Show Records')

class AddInventoryForm(FlaskForm):
    """Implements inventory management form"""
    item_name = StringField('Item Name', validators=[DataRequired(), Length(max=128)])
...
class ManageEmployeeForm(FlaskForm):
    ...
    job_type = SelectField('New Job Type', choices=[], validators=[DataRequired()])
